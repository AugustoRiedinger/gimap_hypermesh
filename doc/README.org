...
On the left side, change =comps= to =elems=. Now, click on =elems= and select =by config=. On =config \==, select =quad4=:

#+CAPTION:faces2
#+LABEL:fig:faces2
[[file:../images/faces2.png]]

You can now press =select entities=. It will redirect you to the previous menu, where you can now press =find faces=.
